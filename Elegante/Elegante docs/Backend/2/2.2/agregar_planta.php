        ...
        <div id="main-menu" class="main-menu collapse navbar-collapse">
            <ul class="nav navbar-nav">
                ...
                <li class="menu-item-has-children dropdown">
                    ...
                    <ul class="sub-menu children dropdown-menu">
                        <li><i class="fa fa-folder-open-o"></i><a href="../2.1/catastro.php">Catastro</a></li>
                        <li><i class="fa fa-plus-square-o"></i><a href="agregar_planta.php">Agregar plantación</a></li>
                    </ul>
                </li>
                <li class="menu-item-has-children dropdown">
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="card">
                            <div class="card-body">
                                <div class="typo-headers">
                                    <h3 class="pb-2 display-4">Registrar una plantación</h3>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="card">
                            ...
                            <div class="card-body card-block">
                                <button style="float:right;" type="button" onclick="location= '../2.1/catastro.php'" class="btn btn-success"><i class="fa fa-book"></i>&nbsp; Ver Plantaciones</button>

                                <form action="guardar_planta.php" method="GET" class="form-horizontal">
                                    <div class="has-warning form-group col-lg-3">
                                        <label for="cuartel" class=" form-control-label"><strong>Cuartel</strong></label>
                                        <input type="number" id="cuartel" name="cuartel" min="0" placeholder="En numero. (ej. 1)" maxlength="2" minlength="1" required autocomplete="on" class="form-control-success form-control">
                                    </div>
                                    <div class="has-warning form-group col-lg-6">
                                        <label for="patron" class=" form-control-label "><strong>Categoría</strong></label>
                                        <select required name="categoria" id="categoria" class="form-control">
                                            <option disabled selected>-- SELECCIONE --</option>
                                            <?php
                                            include '../../connect/connect.php';

                                            $textaux="SELECT `rut_razon` FROM `cuaderno` WHERE `id_cuaderno`=$id_cuaderno";
                                            $que=mysqli_query($dbc,$textaux);
                                            $valu=0;
                                            while($row=mysqli_fetch_row($que)){
                                                $valu=$row[0];
                                            }
                                            $tex="SELECT `id_categoria`, `id_fundo`, `nombre_cat`, `estado` FROM `categoria` WHERE `id_fundo`=$valu ORDER BY `id_categoria` ASC";
                                            $query=mysqli_query($dbc,$tex);
                                    ...
                                    <div class="has-warning form-group col-lg-6">
                                        <label for="especies" class=" form-control-label"><strong>Especie cultivada</strong></label>
                                        <select required name="especies" id="especies"  class="form-control">
                                        </select>
                                    </div>

                                    <div class="has-warning form-group col-lg-6">
                                        <label for="variedad1" class=" form-control-label"><strong>Variedad</strong></label>
                                        <select required name="variedad1" id="variedad1"  class="form-control">
                                        </select>
                                    </div>

                                    <div class="has-warning form-group col-lg-3">
                                        <label for="año" class=" form-control-label"><strong>Año(s) de la plantación</strong></label>
                                        <input type="text" required id="año" name="año" maxlength="20" class="form-control" placeholder="(ej.2005-2009)">
                                    </div>



                                    <div class="has-warning form-group col-lg-6">
                                        <label for="patron" class=" form-control-label"><strong>Patrón de cultivo</strong></label>
                                        <select name="patron" id="patron" class=" form-control">
                                            <option disabled selected>Selecciona un patrón de cultivo</option>
                                            <?php
                                            include '../../connect/connect.php';
                                            ...
                                            $tex="SELECT `id_patron`, `id_fundo`, `nombre`, `estado` FROM `patron` WHERE `id_fundo`=$valu";
                                            $query=mysqli_query($dbc,$tex);
                                            while($row=mysqli_fetch_row($query)){
                                                echo '<option value="'.$row[0].'">'.$row[2].'</option>';
                                            }
                                    ...
                                    <div class="has-warning form-group col-lg-5">
                                        <label for="has" class=" form-control-label"><strong>Hectáreas</strong></label>
                                        <input type="number" step="0.01" required id="has" name="has" class="form-control" placeholder="Decimal separado por un punto. (ej. 2.5 )">
                                    </div>

                                    <div class="has-warning form-group col-lg-9" >
                                        <label for="densidad" class=" form-control-label"><strong>Densidad (SH x EH)</strong></label>
                                        <div style="display: flex;"><input style="margin-right: 5px;" type="number" step="0.01" required id="densidad" name="densidad" class="form-control" placeholder="SH (decimal separado por punto, ej. 2.5)"> <h3>X</h3>
                                            <input style="margin-left: 5px;" type="number" step="0.01" required id="densidad2" name="densidad2" class="form-control" placeholder="EH (decimal separado por punto, ej. 2.5)"></div>
                                    </div>


                                    <div class="has-warning form-group col-lg-5">
                                        <label for="plantas_has" class=" form-control-label"><strong>Plantas por hectárea</strong></label>
                                        <input type="number" required id="plantas_has" step="0.1" name="plantas_has" class="form-control" placeholder="(Decimal separado por punto. Ej. 254.8)">
                                    </div>

                                    <div class="has-warning form-group col-lg-4">
                                        <label for="plantas" class=" form-control-label"><strong>N° Plantas</strong></label>
                                        <input type="number" required id="plantas" name="plantas" class="form-control" placeholder="ingresa un numero Ej. 1500)">
                                    </div>
                                            ...
                                            <div class="form-check">
                                                <div class="checkbox">
                                                    <label for="checkbox1" class="form-check-label ">
                                                        <input disabled type="checkbox" id="checkbox1" name="checkbox1" value="1" class="form-check-input">Aspersión
                                                    </label>
                                                </div>
                                                <div class="checkbox">
                                                    <label for="checkbox2" class="form-check-label ">
                                                        <input disabled type="checkbox" id="checkbox2" name="checkbox2" value="2" class="form-check-input">Goteo
                                                    </label>
                                                </div>
                                                <div class="checkbox">
                                                    <label for="checkbox3" class="form-check-label ">
                                                        <input disabled type="checkbox" id="checkbox3" name="checkbox3" value="3" class="form-check-input"> Tradicional
                                                    </label>
                                                </div>
                        ...
                        <br><br>
                                <button type="submit"  class="btn btn-success btn-lg btn-block">CREAR PLANTACIÓN</button>
                                </form>
                            </div>
                        </div>
                    </div>
    ...
    $(document).ready(function(){
        ...
        $("#especies").change(function(){
            ...
            $.ajax ({
                ...
                success: function(html){
                    $("#variedad1").html(html);
                    $("#variedad1").removeAttr("disabled");
                }
            });
        });


        $("#cuartel").change(function(){
            var id=$(this).val();
            $.ajax ({
                type: "GET",
                url: "cuarteles.php",
