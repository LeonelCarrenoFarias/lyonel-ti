...
header('Content-Type: application/json');
    ...
    $c2="SELECT `Id_Herramienta` FROM `det_herramienta` WHERE `det_herramienta`.`Id_Adqui`=$ide";

    $c3="UPDATE `det_herramienta` SET `Estado`=0 WHERE `Id_Adqui`=$ide";

    $q2=mysqli_query($dbc,$c2); //obtener id de materiales involucrados
    while ($r1=mysqli_fetch_row($q2)){

        $c4="UPDATE `herramienta` SET `Estado`=0 WHERE `Id_Herramienta`=$r1[0]";
