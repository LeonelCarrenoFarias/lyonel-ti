...
if($stock_posterior>=0) {
    ...
    $monto = intval(floatval($cantidad) * floatval($valor));
    $monto*=1.19;
    $monto*=1.20;
    $mn=intval(round($monto));
    $fecha = date('Y-m-d');

    $tt = "INSERT INTO `det_sal_materiales`(`Code`, `Id_Centro`, `Id_Det_Sal_Materiales`, `Detalle`, `Cantidad`, `Monto`, `Fecha_Salida`, `Estado`,`Responsable`) VALUES ($id,'$centro',$id6,'$detalle',$cantidad,$mn,'$fecha',1,'$nombre')";
    ...
    if($q && $q2){
       ...
       echo '<script language="javascript">
            location.href ="Salida.php";alert("Venta registrada exitosamente.");
            </script>'; 
    }else{
        mysqli_close($dbc);
